...
const LibSidebar=()=>{
    return <>
        <div className={'lib-sidebar text-blue-900 border-t border-amber-700'}>
...
useEffect(() => {
        ...
        <table className={'h-fit'}>
            ...
            <tr className={'border-b border-amber-700 '}>
                ...
                {headers.map((header: any, index:any) => {
                  ...
                  return  <th key={index} className={'px-4'} >{header}</th>
                })}
            </tr>
            </thead>
    ...
    return (
        <>
        <div className={'wrapper w-full z-50'}>
            <div className={'header flex justify-between px-6 py-2 '}>
                <h1>My Books</h1>
                <p>Toolbar</p>
            </div>

            <div className={'wrapper flex z-50'}>
                <div className={'flex flex-col gap-4 w-1/6 bg-slate-300 bg-opacity-60 text-xs'}>
                    <LibSidebar/>
                </div>
                <div className={'flex flex-col gap-4 w-5/6 z-50 bg-none '}>
                    <RowCardSlider>
                        <ColumnSlider>
                        {/*// @ts-ignore*/}
